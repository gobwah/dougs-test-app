...
import {
  ...
  sumMovementAmounts,
} from '../movements/utils/movement.util';
...
export class BalanceService {
   ...
   */
  private validateSubsequentBalances(
    balances: Balance[],
    movements: Movement[],
  ): ValidationReason[] {
   ...
   */
  private checkMovementsAfterLastBalance(
    balances: Balance[],
    movements: Movement[],
  ): ValidationReason | null {
    if (balances.length === 0) {
    ...
    if (movementsAfterLastBalance.length > 0) {
      const totalAfterLastBalance = sumMovementAmounts(
        movementsAfterLastBalance,
      );
      return {
  ...
  validateBalances(
    ...
    const firstBalanceError = this.validateFirstBalance(balances[0], movements);
    if (firstBalanceError) {
      reasons.push(firstBalanceError);
    }

    const subsequentErrors = this.validateSubsequentBalances(
      balances,
      movements,
    );
    reasons.push(...subsequentErrors);

    const missingTransactionError = this.checkMovementsAfterLastBalance(
      balances,
      movements,
    );
